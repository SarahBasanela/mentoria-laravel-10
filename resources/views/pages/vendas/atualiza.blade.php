...
@section('content')
    ...
    <form class="form" method="POST" action={{ route('atualizar.cliente', $findCliente->id) }}>
        ...
        <h1 class="h2">Editar Cliente</h1>
            ...
            <input id="nome" value="{{ isset($findCliente->nome) ? $findCliente->nome : old('nome') }}" type="text" class="form-control @error('nome') is-invalid @enderror " name="nome">
            @if ($errors->has('nome'))
        ...
        <div class="mb-3">
            ...
            <input id="email" value="{{ isset($findCliente->email) ? $findCliente->email : old('email') }}"  class="form-control @error('email') is-invalid @enderror " name="email">
            @if ($errors->has('email'))
